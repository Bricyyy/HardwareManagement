...
public class UserOrderHistory extends javax.swing.JFrame {
    ...
    public UserOrderHistory(String uname, String uid){
        initComponents();
        checkConnections();
        columnHider();
    ...
    public ArrayList<orderList> orderLists(String uid) {
        ArrayList<orderList> orderT = new ArrayList<>();
        try{
            String q2 = "SELECT * FROM UserOrderHistory WHERE uid = '"+uid+"'";
            rs = st.executeQuery(q2);
            orderList olist;
            while (rs.next()) {
                olist = new orderList(rs.getInt("uid"), rs.getString("recID"), rs.getString("utotal"), rs.getString("paid"), rs.getString("bal"), rs.getString("date"), rs.getString("time"));
                orderT.add(olist);
            }
        }
        catch(SQLException e){
    ...
    public void showOrder(String uid) {
        ArrayList<orderList> list = orderLists(uid);
        DefaultTableModel model = (DefaultTableModel) orderTable.getModel();
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            row[2] = list.get(i).getTotalAmount();
            row[3] = list.get(i).getPaidAmount();
            row[4] = list.get(i).getBalanceAmount();
    ...
    public ArrayList<purchaseList> purchasedLists() {
        String dateShow = orderTable.getValueAt(orderTable.getSelectedRow(), 5).toString();
        String timeShow = orderTable.getValueAt(orderTable.getSelectedRow(), 6).toString();
    ...
    private void initComponents() {
        ...
        userAcc.setFont(new java.awt.Font("Arial", 1, 18)); // NOI18N
        userAcc.setForeground(new java.awt.Color(210, 180, 64));
        userAcc.setText("CONSUMER");
        ...
        orderTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "User ID", "Order", "Total Price", "Paid Cash", "Balance", "Date", "Time"
            }
        ) {
